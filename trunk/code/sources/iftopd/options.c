...
void options_set_defaults() {
    ...
    options.dnsresolution = 0;/* 决定是不是对IP进行反解DNS    ，值 为0不解析 */
...
#endif
    options.showbars = 1;
    options.showports = OPTION_PORTS_OFF;/* (off|source-only|destination-only|on)端口号的显示模式 */
    options.aggregate_src = 0;
    options.aggregate_dest = 0;
    options.paused = 0;
    options.showhelp = 0;
    options.bandwidth_in_bytes = 0;
    options.sort =  OPTION_SORT_DIV1 ;/*OPTION_SORT_DIV2  */
    ...
    options.no_curses = 1;/*  值为1表示关闭ncurses画图 */
    options.num_lines = 500;/* 输出前500条连接上下行速度 */
	options.speed_second_time = 1; /* 默认是1  代表单位是秒 ，当值为2时，代表单位是分钟*/
	options.interval_seconds_time = 1;/*采集数据频率， 默认是1 second */

#if 1
	 int i = 70 + 9 + 1;
	 options.config_file = xmalloc(i);
	 snprintf(options.config_file,i,"/root/.iftopdc",s); 
...
/* -c config file 
		指定配置文件。如果没有指定，如果存在~/.iftopdrc，则使用~/.iftopdrc。配置规则可以参看下面的描述。
	*/
    ...
    if(s != NULL) {
        int i = strlen(s) + 9 + 1;
        options.config_file = xmalloc(i);
        snprintf(options.config_file,i,"%s/.iftopdc",s);  
    }
    else  { 
        options.config_file = xstrdup("iftopdc");   
    }
...
void correction_parameter()
{
	...
	if(2 == options.speed_second_time)
	{
		options.sort == OPTION_SORT_DIV3;/* 当  解决排问题 */
	}

#if 0
	if(2 == options.speed_second_time)
	{
		if(options.interval_seconds_time < 30)
		{	
			options.interval_seconds_time = 30;/* 当单位是min 时 ，采集数据周期应该在应该在30s以上， */
		}
	}
#endif 
...
void options_make() {
	...
	options_config_get_int("speed-second-time", &options.speed_second_time);
	options_config_get_int("interval-seconds-time", &options.interval_seconds_time);
